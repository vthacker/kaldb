...
import com.slack.kaldb.elasticsearchApi.searchRequest.SearchRequest;
import com.slack.kaldb.elasticsearchApi.searchResponse.HitsMetadata;
import com.slack.kaldb.elasticsearchApi.searchResponse.SearchResponse;
...
public class ElasticsearchApiService {
  ...
  public HttpResponse multiSearch(String postBody) throws IOException {
    List<SearchRequest> requests = SearchRequest.parse(postBody);
  ...
  private SearchResponse doSearch(SearchRequest request) throws IOException {
    ...
    return new SearchResponse.Builder().hits(hitsMetadata).status(200).build();
  }
