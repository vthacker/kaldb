...
public class SearchRequest {
  ...
  public static List<SearchRequest> parse(String postBody) throws JsonProcessingException {
    ObjectMapper om =
    ...
    for (List<String> pair : Lists.partition(Arrays.asList(postBody.split("\n")), 2)) {
      ...
      // by default grafana formats an empty query as "*" in the request payload, this un-does that
      String queryString = body.get("query").findValue("query").asText();
